...
CREATE INDEX idx_sources_medium ON sources(medium);
...
CREATE UNIQUE INDEX idx_items_dedup ON items(source_id, url);
...
-- Create indexes for item_topics
CREATE INDEX idx_item_topics_item_id ON item_topics(item_id);
CREATE INDEX idx_item_topics_topic ON item_topics(topic);
CREATE INDEX idx_item_topics_topic_item ON item_topics(topic, item_id);
